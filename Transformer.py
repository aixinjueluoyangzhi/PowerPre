...
logger = setup_logging(modle='Transformer')
...
class TransformerForecast(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_len, num_heads=4, num_layers=2, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = x[:, -1, :]                    # 取最后一个时间步
        return self.output(x)             # (B, output_len)
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        model_s = TransformerForecast(input_dim=train_scaled.shape[1], hidden_dim=hidden_dim, output_len=pred_len_short)
        ...
        plot_prediction(pred_s, true_s, "Short-Term Power Forecast (90 days)", "Transformer_short_term_forecast")
        # pred_s:516
        ...
        model_l = TransformerForecast(input_dim=train_scaled.shape[1], hidden_dim=hidden_dim, output_len=pred_len_long)
        ...
        plot_prediction(pred_l, true_l, "Long-Term Power Forecast (365 days)", "Transformer_long_term_forecast")
        # pred_l:241
